...
mesh = os.path.join('../data/geom/synthetic_mesh.nc')

# Paths to use for parameter design
X_physical = os.path.join(exp_dir, '{exp}_test_physical.csv'.format(exp=exp))
X_log = os.path.join(exp_dir, '{exp}_test_log.csv'.format(exp=exp))
X_standard = os.path.join(exp_dir, '{exp}_test_standard.csv'.format(exp=exp))
table = 'test.dat'
