...
bh_xy = [-205723.84235624867, -2492713.333133503,]
...
cax = ax.inset_axes((1.02, 0.1, 0.025, 0.63))
...
for node_index in plot_nodes:
    ...
    ax.plot(mesh['x'][node_index]/1e3, mesh['y'][node_index]/1e3, 
        marker='s', color=colors[iii], zorder=5, markeredgecolor='w',
...
scale = Rectangle(xy=(xmin+0, ymin-8), width=50, height=1, zorder=15)
spc = PatchCollection([scale], facecolor='k', clip_on=False)
ax.add_collection(spc)
ax.text(xmin+0+0.5*50, ymin-8+2.5, '50 km', ha='center', va='bottom')

leg = ax.legend(bbox_to_anchor=(0, 0.1, 0.5, 0.35), 
    frameon=False, loc='upper left', borderpad=0, borderaxespad=0)
...
fig.subplots_adjust(left=0.02, bottom=0.025, right=0.85, top=0.975)
fig.savefig('greenland_domain_summary_EGU_simple_00.png', dpi=400)



ax.plot(bh_xy[0]/1e3, bh_xy[1]/1e3, '^', markersize=ms*1.25, label='Borehole', zorder=10,
    color='r', markeredgecolor='w', markeredgewidth=mlw)

leg.remove()
leg = ax.legend(bbox_to_anchor=(0, 0.1, 0.5, 0.35), 
    frameon=False, loc='upper left', borderpad=0, borderaxespad=0)

fig.savefig('greenland_domain_summary_EGU_simple_01.png', dpi=400)