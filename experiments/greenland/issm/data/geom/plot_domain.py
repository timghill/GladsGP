...
bed = np.vstack(np.load('greenland_bed.npy'))
surf = np.vstack(np.load('greenland_surface.npy'))
thick = surf - bed
bed[thick<50] = surf[thick<50] - 50
thick = surf - bed

# Compute triangulation
md = read_netCDF('greenland_bamg.nc')
md = meshconvert(md, md.mesh.elements, md.mesh.x, md.mesh.y)
triangulation = tri.Triangulation(md.mesh.x/1e3, md.mesh.y/1e3, md.mesh.elements-1)

# Find interesting nodes

# Find interesting nodes
x1 = -210e3
x1mask = np.abs(md.mesh.x - x1)<2e3
bedmask = bed.copy()
bedmask[~x1mask] = np.nan
node1 = np.nanargmin(bedmask.flatten())
print('Node 1:', node1)
print(md.mesh.x[node1]/1e3, md.mesh.y[node1]/1e3, bed[node1], surf[node1])
...
print('Node 2:', node2)
print(md.mesh.x[node2]/1e3, md.mesh.y[node2]/1e3, bed[node2], surf[node2])
...
print('Node 3:', node3)
print(md.mesh.x[node3]/1e3, md.mesh.y[node3]/1e3, bed[node3], surf[node3])

plot_nodes = [node1, node2, node3]

# area_ele = GetAreas(md.mesh.elements, md.mesh.x, md.mesh.y)/1e6
...
for ax in (ax1, ax2):
    ...
    for node_index in plot_nodes:
        ...
        ax.plot(md.mesh.x[node_index]/1e3, md.mesh.y[node_index]/1e3, 
            marker='s', color=colors[iii], zorder=10, markeredgecolor='w',
            markersize=ms, markeredgewidth=mlw)
        ax.plot(md.mesh.x[moulin_indices]/1e3, md.mesh.y[moulin_indices]/1e3,
...
zmax = 1850
surf0 = surf[:, 0]
xmax = np.max(md.mesh.x[surf0<=zmax])/1e3
ymax = np.max(md.mesh.y[surf0<=zmax])/1e3
xmin = np.min(md.mesh.x[surf0<=zmax])/1e3
ymin = np.min(md.mesh.y[surf0<=zmax])/1e3
ax2.set_xlim([xmin, xmax])
ax2.set_ylim([ymin, ymax])

ax1.set_xlim([xmin, np.max(md.mesh.x)/1e3])
...
scale2 = Rectangle(xy=(md.mesh.x.max()/1e3-10-100, ymax-10), width=100, height=5, zorder=15)
spc2 = PatchCollection([scale2], facecolor='k', clip_on=False)
ax1.add_collection(spc2)
ax1.text(md.mesh.x.max()/1e3-10-0.5*100, ymax-10+5+2, '100 km', ha='center', va='bottom')
...
outline = np.loadtxt('greenland_outline.csv', skiprows=1, quotechar='"', delimiter=',')
